    ...
    <motion.nav initial={{ y: -100 }} animate={{ y: 0 }} transition={{ duration: 0.8, ease: [0.16, 1, 0.3, 1] }}
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-10">
        <div className="flex items-center justify-between h-20">
          <HashLink smooth to="/#hero" className="relative group">
        ...
        {mobileOpen && (
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }}
            ...
            <div className="px-6 py-6 flex flex-col gap-4">
              {links.map((link) => (
                <HashLink key={link.href} smooth to={link.href} onClick={() => setMobileOpen(false)}
                  className="text-[#A3A3A3] hover:text-[#F97316] transition-colors py-2 font-['DM_Sans',sans-serif]">{link.label}</HashLink>
              ))}
              <HashLink smooth to="/#contacto" onClick={() => setMobileOpen(false)}
                className="mt-2 px-6 py-3 bg-[#F97316] text-[#0A0A0A] font-semibold rounded-full text-center hover:bg-[#EA580C] transition-all font-['DM_Sans',sans-serif]">Cotizar Ahora</HashLink>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
